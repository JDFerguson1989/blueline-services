              ...
              <div>
                <ul className="flex flex-col gap-2 min-w-[220px] whitespace-nowrap">
                  {SERVICE_AREAS.map((area) => (
                    <li key={area} className="flex items-center gap-2 group hover:translate-x-1 transition-all duration-200">
                      <svg className="w-4 h-4 text-blue-400 flex-shrink-0 group-hover:scale-110 transition-transform duration-200" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z" />
                      </svg>
                      {LINKED_PATHS[area as keyof typeof LINKED_PATHS] ? (
                        <a href={LINKED_PATHS[area as keyof typeof LINKED_PATHS]} className="font-display font-bold text-white text-[15px] tracking-wide hover:text-blue-400 hover:underline transition-all duration-200">
                          {area}
                        </a>
                      ) : (
                ...
                <p className="text-gray-300 text-sm mt-6 leading-relaxed max-w-prose">
                  BlueLine Services provides pressure washing, soft washing, driveway cleaning, house washing, roof cleaning, and exterior cleaning across Sargent, Lake Jackson, Freeport, Bay City, Sweeny, Brazoria, Cedar Lake, Cedar Lane, and nearby Gulf Coast communities.
                </p>
              </div>
